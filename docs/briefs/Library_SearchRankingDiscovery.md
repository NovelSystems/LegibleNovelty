...
- **RESOLVED (subsequent round): AI-attestation is now enforced at submission.** `submitForReview` rejects the draft → pending-review transition when `ai_attestation` is null (Section 9.4: "must declare ... before the system accepts it"), and migration `20260803150000_backfill_null_ai_attestation` normalized pre-enforcement submitted/published rows to `ai_pipeline` (1× — the lowest multiplier, which also preserves the exact 1× ranking a null already had, so no advantage is gained). The column stays nullable on purpose: `null` is the correct state for a Draft that hasn't reached the submission gate, and Drafts are never ranked. See the README open-items for the full nullable-vs-NOT-NULL reasoning and the `ai_pipeline` permanent-lock caveat.
