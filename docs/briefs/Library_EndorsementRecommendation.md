...
- **Endorsement scoped to `ve_status` only.** Section 4.3 grants LNC-holders the same endorsement ability, but this brief's acceptance criteria scope creation to `ve_status = true`, and nothing sets `lnc_status` until Certification Center ships. Widening the live check to `ve_status || lnc_status` is the one-line change to make then.
...
- **"Edited under you" line-count delta left null.** The warning fires correctly; a real PR-style line delta needs a pre-edit content snapshot Module Editor does not persist (only `last_edited_at`). Filling the number is a Module Editor edit-diff-baseline follow-up.
