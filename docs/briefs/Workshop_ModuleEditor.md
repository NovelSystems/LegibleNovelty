...
- The platform-hosted AI authoring wizard — deferred pending an established contributor base and unresolved pricing model (Section 18).
...
- **Edit scope:** everything editable post-publication except: the seed reference can change but re-triggers Task 3's seed-alignment check; commission association can change either direction; AI attestation locks permanently if the module was created via the AI Pipeline wizard (moot now since the wizard is deferred, but the rule should exist for later); the commission snapshot freezes at original publication regardless of later edits to anything else.
...
- **Unrelated cleanup, tracked here per instruction rather than in the Standing Scores brief it actually belongs to:** `canRestoreEss` in the Seed Editor/Standing Scores branch may now be dead code following the restore-then-grant reordering — confirm whether the "different VE" restoration requirement is still enforced anywhere in the grant path, and if not, either wire it in or delete the unused function.
